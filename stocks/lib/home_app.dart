...
class HomeAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  HomeAppBar({required this.title, this.onSelect});
...
class MySearch extends SearchDelegate<String> {
  ...
  List<Widget> buildActions(BuildContext context) {
    return [
      IconButton(
          ...
          onPressed: () => query = ""), // clear query button
    ];
  }
      ...
      _lastFutureBuilder = FutureBuilder(
        ...
        builder: (BuildContext context, AsyncSnapshot<List<dynamic>> snapshot) {
          if (snapshot.connectionState == ConnectionState.done) {
            return _getRecentListView(snapshot.data);
          } else
            return SizedBox();
        },
      );
  ...
  Widget _getRecentListView(List<dynamic>? list) => ListView.builder(
  ...
  static const _TEXT_STYLE_UNMARKED =
          TextStyle(color: Colors.black), // style for unmarked text
      _TEXT_STYLE_MARKED = TextStyle(
          fontWeight: FontWeight.bold,
          color: Colors.blue); // style for marked text
  ...
  List<TextSpan> _getTextSpans(String text) {
    ...
    if (pos != 0)
      textSpans.add(
          TextSpan(text: text.substring(0, pos), style: _TEXT_STYLE_UNMARKED));
    textSpans.add(TextSpan(
      text: text.substring(pos, pos + query.length),
      style: _TEXT_STYLE_MARKED,
    ));
    if (pos + query.length < text.length)
      textSpans.add(TextSpan(
          text: text.substring(pos + query.length),
          style: _TEXT_STYLE_UNMARKED));
    return textSpans;
  }
}
